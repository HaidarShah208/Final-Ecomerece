      ...
      <div className="h-auto bg-gray-100 pt-5 " style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '', }}>
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
            ...
            return(
              ...
              <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                <div className="mt-5 sm:mt-0">
                  <h2 className="text-lg font-bold text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}><span className='text-fuchsia-600 text-medium me-3'>Title</span> {title}</h2>
                  <p className="mt-1  font-semibold text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}><span className='text-fuchsia-600 text-medium me-3'>Price</span> {price}</p>
            <span className='inline-block bg-slate-700 mt-4 p-1 rounded text-white' >Description</span>
            <p className='p-2 text-start text-sm font-mono'>{description}</p>
          ...
          <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
            ...
            {/* <Modal  /> */}
            <CartModel buyNow={buyNow} name={name} address={address} pincode={pincode} phoneNumber={phoneNumber} setName={setName} setAddress={setAddress} setPincode={setPincode} setPhoneNumber={setPhoneNumber}  />
          </div>
        </div>
      </div>
    </Layout>
